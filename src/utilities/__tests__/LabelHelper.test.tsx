...
import { calculateDigitsTotalInProblem, formatDate } from "../LabelHelper";
...
describe('LabelHelpers: calculateDigitsTotalInProblem', () => {
    it('Subtraction: Should confirm numbers in problem (total)', () => {
        ...
        const totalDigitsInProblem = baseValue.toString().length +
    ...
    it('Fail: missing operator', () => {
        expect(() => calculateDigitsTotalInProblem("12=12")).toThrow(Error('No operator found in fact string'));
    })

    it('Fail: missing equality sign', () => {
        expect(() => calculateDigitsTotalInProblem("12+12")).toThrow(Error('No equality sign found in fact string'));
    })
})
